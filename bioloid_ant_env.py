...
class BioloidAntLikeEnv(gym.Env):
    ...
    def __init__(
        ...
        ctrl_cost_weight: float = 0.01,        # slightly smaller than Ant default
        contact_cost_weight: float = 2e-4,     # scaled for ~0.85kg
        alive_bonus: float = 0.05,
        max_steps: int = 1000,
        # Fall detection tuning
        fall_low_k: float = 0.45,              # z < max(fall_low_abs, fall_low_k*body_height)
        fall_low_abs: float = 0.035,
        fall_low_steps: int = 2,               # consecutive steps under low to declare fall
        tilt_limit_rad: float = 0.261799,      # ~15 degrees
    ) -> None:
        super().__init__()
        ...
        # The action space is now target joint positions in radians.
        # The values are normalized to [-1, 1] and will be scaled to the joint limits.
        self.action_space = spaces.Box(
            low=-1.0, high=1.0, shape=(len(self.joint_names),), dtype=np.float32
        )
        # Joint limits (in radians). These should be verified with the URDF.
        # Assuming symmetric limits for all joints for now.
        self.joint_limits = [(-math.pi / 4, math.pi / 4)] * len(self.joint_names)
        ...
        self._low_count = 0  # consecutive too_low counter for robust fall detection
        self.np_random = np.random.RandomState()

        # Fall detection params
        self.fall_low_k = float(fall_low_k)
        self.fall_low_abs = float(fall_low_abs)
        self.fall_low_steps = int(fall_low_steps)
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        # Reset joint states and set up position control
        for j in self.joint_indices:
            ...
            p.setJointMotorControl2(
                ...
                targetPosition=0,
                force=self.torque_limit,  # Max force
    ...
    def _is_terminated(self) -> bool:
        # Terminate on collapse or excessive tilt, scaled to robot size, with mild hysteresis
        base_pos, base_quat = p.getBasePositionAndOrientation(self.robot_id, physicsClientId=self.client_id)
        base_z = float(base_pos[2])
        roll, pitch, _ = p.getEulerFromQuaternion(base_quat)

        # Thresholds relative to nominal body height (configurable)
        low_thresh = max(self.fall_low_abs, self.fall_low_k * self.body_height_m)
        too_low_raw = base_z < low_thresh
        if too_low_raw:
            self._low_count += 1
        else:
            self._low_count = 0
        # Require N consecutive low-height readings to declare too_low
        too_low = self._low_count >= max(1, self.fall_low_steps)

        too_high = base_z > 2.5 * self.body_height_m
        # Tilt threshold (configurable)
        tilt_limit = self.tilt_limit_rad
        too_tilted = (abs(roll) > tilt_limit) or (abs(pitch) > tilt_limit)
