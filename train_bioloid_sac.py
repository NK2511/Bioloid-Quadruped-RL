    ...
    def on_step(self, ctx: Dict[str, Any]):
        ...
        if "info" in ctx:
            for key, value in ctx["info"].items():
                if key in ["tracking_reward", "wz_penalty", "ctrl_cost", "contact_cost"]:
                    self.writer.add_scalar(f"reward_components/{key}", value, t)
...
def sac_train(env, agent, memory, batch_size, start_steps, num_episodes, max_steps, callbacks: List[Callback], gradient_updates_per_step: int = 1, gui_every=0, eval_interval=0, save_dir="dir_bioloid_ant_like", checkpoint_interval=0, resume_episode=0):
    total_numsteps = 0
    updates = 0
    ...
    for i_episode in range(resume_episode, num_episodes):
        use_gui = (gui_every > 0 and (i_episode % gui_every == 0))
        if use_gui:
        ...
        reward_str = (
            f"{colors.GREEN}Alive: {episode_reward_breakdown['alive_bonus']:.2f}{colors.END} | "
            f"{colors.GREEN}Track: {episode_reward_breakdown['tracking_reward']:.2f}{colors.END} | "
            f"{colors.CYAN}Wz: {episode_reward_breakdown['wz_penalty']:.2f}{colors.END} | "
            f"{colors.RED}Ctrl: {episode_reward_breakdown['ctrl_cost']:.2f}{colors.END} | "
            f"{colors.RED}Contact: {episode_reward_breakdown['contact_cost']:.2f}{colors.END}"
        )

        print(
            f"Ep: {i_episode} | {colors.PURPLE}{COMMAND_NAMES[env.command]:<8}{colors.END} | Steps: {episode_steps} | {colors.BOLD}Score: {episode_reward:.2f}{colors.END} | "
            f"Avg: {avg_score:.2f} | Time: {s//3600:02}:{(s%3600)//60:02}:{s%60:02}"
        )
        print(f"   └ {reward_str}")
...
def evaluate_policy(agent, episodes=3):
    env_eval = CommandedBioloidEnv(render_mode="DIRECT")

    for cmd_id, cmd_name in COMMAND_NAMES.items():
        total_return = 0.0
        for _ in range(episodes):
            state, _ = env_eval.reset()
            env_eval.set_command(cmd_id)
        ...
        print(f"  Eval {cmd_name}: {avg_return:.2f}")

    env_eval.close()

def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--gui-every", type=int, default=0)
    ...
    p.add_argument("--start-steps", type=int, default=100_000)
    ...
    p.add_argument("--torque-limit", type=float, default=1.0, help="Max per-joint torque (N·m) for Bioloid joints")
